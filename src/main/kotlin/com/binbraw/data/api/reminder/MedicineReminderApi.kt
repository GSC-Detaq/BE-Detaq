...
object MedicineReminderApi : KoinComponent {
                ...
                AllMedicineReminderResponse(
                    ...
                    data = transaction {
                        medReminderTable.select {
                            medReminderTable.uid eq uid and(medReminderTable.status_code eq 1)
                        }.mapNotNull {
    ...
    fun Route.endStatusMedicineReminder(path:String){
        get(path){
            val uid = call.principal<JWTPrincipal>()!!.payload.getClaim("uid").asString()
            val reminder_id = call.parameters["reminder_id"] ?: ""

            transaction {
                medReminderTable.update({
                    medReminderTable.reminder_id eq UUID.fromString(reminder_id) and(medReminderTable.uid eq uid)
                }) {
                    it[medReminderTable.status_code] = 2
                }
            }.let {
